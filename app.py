...
app = Flask(__name__)
app.config['SECRET_KEY'] = 'dev_secret_key' # Change this for production
...
def contact():
    ...
    if request.method == 'POST':
        ...
        if full_name and email:
            new_contact = Contact(full_name=full_name, email=email, mobile=mobile, city=city)
            db.session.add(new_contact)
            db.session.commit()
            flash('Your quote request has been sent!', 'success')
        else:
            flash('Please fill in all fields.', 'danger')
            
    return redirect(url_for('index'))

# FIXED: Subscribe Route
...
def add_project():
    if request.method == 'POST':
        ...
        project = Project(name=request.form['name'], description=request.form['description'], image_file=filename)
...
def add_client():
    if request.method == 'POST':
        pic = request.files['image']
        filename = save_and_crop_image(pic, (300, 300))
        client = Client(name=request.form['name'], designation=request.form['designation'], description=request.form['description'], image_file=filename)
